...
function loadActiveLeagues(): number[] {
  try {
    ...
    if (Array.isArray(parsed) && parsed.length > 0) {
      // Migrate 254 to 304, 370 to 312, and ensure 128 (Argentina) is included
      const mapped = parsed.map((id: number) => id === 254 ? 304 : id === 370 ? 312 : id);
      if (!mapped.includes(128)) {
        mapped.push(128);
      }
      return mapped;
    }
...
function renderOpportunitiesCenter(liveMatches: any[] = state.liveMatches) {
  ...
  filtered.forEach((opp, rankIdx) => {
    ...
    const oppKey = getOpportunityPushKey(opp);
    const isPushActive = isOpportunityPushActive(oppKey);
    ...
    card.className = 'opp-opportunity-card opp-card-active-trade';
    card.style.background = 'rgba(15, 23, 42, 0.85)';
    card.style.border = isCurrentlyOperating ? '2px solid #06b6d4' : (opp.tier === 'PREMIUM' ? '2px solid #10b981' : (opp.tier === 'FUERTE' ? '2px solid #38bdf8' : `1.5px solid ${opp.tierColor}60`));
    ...
    const defaultBorder = isCurrentlyOperating ? '2px solid #06b6d4' : (opp.tier === 'PREMIUM' ? '2px solid #10b981' : (opp.tier === 'FUERTE' ? '2px solid #38bdf8' : `1.5px solid ${opp.tierColor}60`));

    card.addEventListener('mouseenter', () => {
      card.style.transform = 'translateY(-2px)';
      if (!isCurrentlyOperating) card.style.borderColor = opp.tierColor;
    });
    card.addEventListener('mouseleave', () => {
      card.style.transform = 'translateY(0)';
      if (!isCurrentlyOperating) card.style.border = defaultBorder;
    });
    ...
    card.innerHTML = `
      ...
      <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 0.35rem; background: rgba(255,255,255,0.02); padding: 0.4rem; border-radius: 6px; font-size: 0.65rem; text-align: center; margin-top: 0.35rem; align-items: stretch;">
        <!-- Racha Actual destacada con alta visibilidad y corte previo -->
        <div style="background: linear-gradient(135deg, rgba(249, 115, 22, 0.22) 0%, rgba(239, 68, 68, 0.28) 100%); border: 1.5px solid #f97316; border-radius: 6px; padding: 0.3rem 0.2rem; box-shadow: 0 0 12px rgba(249, 115, 22, 0.4); display: flex; flex-direction: column; justify-content: center;">
          ...
          ${opp.streakPrevious > 0 ? `<span style="font-size: 0.58rem; color: #cbd5e1; margin-top: 0.15rem; font-weight: 800; border-top: 1px dashed rgba(249, 115, 22, 0.5); padding-top: 0.15rem;">✂️ ${lang.streaks.brokenAt} <strong style="color: #38bdf8;">${opp.streakPrevious}</strong> ${lang.streaks.matchesUnit}</span>` : ''}
        </div>
      ...
      <div style="display: flex; justify-content: space-between; align-items: center; gap: 0.5rem; margin-top: 0.35rem;">
        ...
        <button class="btn-push-alert ${isPushActive ? 'active-push' : ''}" data-opp-key="${oppKey}" data-league-id="${opp.leagueId}" title="${isPushActive ? 'Alerta Push Activada para este partido (clic para desactivar)' : (lang.opportunitiesCenter?.pushAlertTitle || 'Activar Alerta Push')}">
          ${isPushActive ? '🔔 Activado' : '🔔'}
        </button>
      </div>
    ...
    // Handle Push Notification Bell Button Click (Toggle individual per banner)
    const pushBellBtn = card.querySelector('.btn-push-alert');
    if (pushBellBtn) {
      pushBellBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        toggleOpportunityPush(opp);
      });
    }
...
function updateStaticLanguageTexts() {
  ...
  if (planSelectEl) {
    ...
    if (optFree) optFree.textContent = state.currentLang === 'en' ? '⚪ INICIO Plan ($0/mo)' : (state.currentLang === 'pt' ? '⚪ Plano INÍCIO ($0/mês)' : (state.currentLang === 'gn' ? '⚪ Plan INICIO ($0/jasy)' : '⚪ Plan INICIO ($0/mes)'));
    if (optTrial) optTrial.textContent = state.currentLang === 'en' ? '🧪 TRIAL (3-Day Trial)' : '🧪 TRIAL (Prueba 3 Días)';
    if (optPro) optPro.textContent = state.currentLang === 'en' ? '🔵 PRO (Quantitative)' : '🔵 PRO (Cuantitativo)';
    if (optVip) optVip.textContent = state.currentLang === 'en' ? '👑 VIP Plan ($2 / 7 days)' : (state.currentLang === 'pt' ? '👑 Plano VIP ($2 / 7 dias)' : (state.currentLang === 'gn' ? '👑 Plan VIP ($2 / 7 ára)' : '👑 Plan VIP ($2 / 7 días)'));
  }
...
function getOpportunityPushKey(opp: any): string {
  if (!opp) return '';
  return `${opp.leagueId}_${opp.marketKey || 'all'}`;
}

function loadPushNotificationPreferences(): Record<string, PushConfigEntry> {
  try {
...
function savePushNotificationPreferences(prefs: Record<string, PushConfigEntry>) {
  try {
    localStorage.setItem(STORAGE_KEY_PUSH_NOTIF, JSON.stringify(prefs));
  } catch (e) {}
}

function isOpportunityPushActive(oppKey: string): boolean {
...
function updateOpportunityPushButtonsUI(oppKey: string, isActive: boolean) {
  ...
  btns.forEach(btn => {
    if (isActive) {
      ...
      btn.title = 'Alerta Push Activada para este partido (clic para desactivar)';
    } else {
...
function toggleOpportunityPush(opp: any): void {
  ...
  if (!oppKey) return;
  const prefs = loadPushNotificationPreferences();
  const isCurrentlyActive = !!prefs[oppKey];
  ...
  if (isCurrentlyActive) {
    ...
    prefs[oppKey] = {
      ...
      fixtureName: opp.fixtureName,
      marketLabel: opp.marketLabel,
      matchTimestamp: opp.sortTimestamp || Date.now(),
      notify10Min: true,
      notifyLive: true,
      notifyGoal: true,
      notifyFt: true,
    ...
    savePushNotificationPreferences(prefs);
    updateOpportunityPushButtonsUI(oppKey, true);
...
function setupPushNotificationModule() {
  ...
  if (saveBtn) {
    saveBtn.addEventListener('click', async () => {
      ...
      if (currentTarget) {
        const currentOppKey = getOpportunityPushKey(currentTarget);
        activePush[currentOppKey] = {
          oppKey: currentOppKey,
          leagueId: currentTarget.leagueId,
          marketKey: currentTarget.marketKey,
          leagueName: currentTarget.leagueName,
          fixtureName: currentTarget.fixtureName,
          marketLabel: currentTarget.marketLabel,
          matchTimestamp: currentTarget.sortTimestamp || Date.now(),
        ...
        updateOpportunityPushButtonsUI(currentOppKey, true);
...
(window as any).openPushNotificationModal = openPushNotificationModal;
